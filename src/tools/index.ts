...
import { registerCheckCuaCodeAudit } from "./checkCuaCodeAudit.js";

/** 注册全部 5 个工具到 server。每个工具闭包持有已绑定该请求 key 的 BackendClient。 */
export function registerAllTools(server: McpServer, backend: BackendClient): void {
  ...
  registerCheckCuaCodeAudit(server, backend);
